...
function buildList(jobs) {
    ...
    for (let job of jobs) {
        ...
        html +=`<div>
        <input type="radio" id="${job.occupation.slice(1, 4)}" name="careerOptions" value="${job.salary}">
        <label for="${job.occupation.slice(1, 4)}" >${job.occupation} - $${job.salary.toLocaleString('en-US')}</label>
        </div>
        `;
